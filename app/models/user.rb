class User < ApplicationRecord
  ...
  def set_password(plain_text_password)
    update(password: BCrypt::Password.create(plain_text_password))
  end

  def check_password(plain_test_password)
    BCrypt::Password.new(password) == plain_test_password
  end
  ...
  def hash_password
    self.password = BCrypt::Password.create(password) if password.length > 0
  end
